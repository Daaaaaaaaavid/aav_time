...
export default function TimeEntryItem({ entry, onDelete, onEdit }: TimeEntryItemProps) {
  ...
  const durationText = entry.duration ? `${entry.duration} min` : 'Läuft...';
  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-md hover:bg-gray-100">
      ...
      {onEdit && (
        <button onClick={() => onEdit(entry)} className="px-2 py-1 text-blue-600 hover:bg-blue-50 rounded text-sm">
          Bearbeiten
        </button>
      )}
